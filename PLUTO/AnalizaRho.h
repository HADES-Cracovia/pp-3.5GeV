...
// Mon Apr  6 17:36:08 2020 by ROOT version 5.34/34
// from TTree data/Event data
// found on file: pK+L1520.root
//////////////////////////////////////////////////////////
...
#ifdef AnalizaRho_cxx
AnalizaRho::AnalizaRho(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("pK+L1520.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("pK+L1520.root");
      }
...
AnalizaRho::~AnalizaRho()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t AnalizaRho::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t AnalizaRho::LoadTree(Long64_t entry)
{
...
void AnalizaRho::Init(TTree *tree)
{
...
void AnalizaRho::Show(Long64_t entry)
{
...
Int_t AnalizaRho::Cut(Long64_t entry)
{
