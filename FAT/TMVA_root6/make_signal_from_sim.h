...
#ifndef make_signal_from_sim_h
#define make_signal_from_sim_h
...
public :
   ...
   make_signal_from_sim(TTree *tree=0);
   virtual ~make_signal_from_sim();
...
#ifdef make_signal_from_sim_cxx
make_signal_from_sim::make_signal_from_sim(TTree *tree) : fChain(0) 
{
  ...
  if (tree == 0)
    {
      ...
      //chain->Add("/lustre/hades/user/knowakow/PP/FAT/PPIMPIPPIM_sim/S1385pK0_Rafal.root/ppimpippim");
      //chain->Add("/lustre/hades/user/knowakow/PP/FAT/PPIMPIPPIM_sim/SDppK0_Rafal.root/ppimpippim");
      //chain->Add("/lustre/hades/user/knowakow/PP/FAT/PPIMPIPPIM_sim/LDppK0_Rafal.root/ppimpippim");
      //chain->Add("/lustre/hades/user/knowakow/PP/FAT/PPIMPIPPIM_sim/pp_ppimpippim_sim.root/ppimpippim");
      chain->Add("/lustre/hades/user/knowakow/PP/FAT/PPIMPIPPIM_sim/pp_Lpippim.root/ppimpippim");
      tree = chain;
    }
...
make_signal_from_sim::~make_signal_from_sim()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t make_signal_from_sim::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t make_signal_from_sim::LoadTree(Long64_t entry)
{
...
void make_signal_from_sim::Init(TTree *tree)
{
...
Bool_t make_signal_from_sim::Notify()
{
...
void make_signal_from_sim::Show(Long64_t entry)
{
...
Int_t make_signal_from_sim::Cut(Long64_t entry)
{
...
// returns -1 otherwise.
  if(
     ...
     //&& pip_sim_id==8
     //&& pip_sim_parentid==0
     && m_inv_p_pim_pip_pim<1600
     )
    return 1;
  else
    return 0;
}
#endif // #ifdef make_signal_from_sim_cxx
