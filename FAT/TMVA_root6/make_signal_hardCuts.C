#define make_signal_hardCuts_cxx
#include "make_signal_hardCuts.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
void make_signal_hardCuts::Loop()
{
  //   In a ROOT session, you can do:
  //      Root > .L make_signal_hardCuts.C
  //      Root > make_signal_hardCuts t
  ...
  TFile* outputFile = new TFile("hardCuts_signal.root","recreate");
  if ( outputFile == 0 )
    {
  ...
  //signal->Branch("m_inv_pip_pim",&m_inv_pip_pim);
  //signal->Branch("miss_mass_kp",&miss_mass_kp);
  ...
  //background->Branch("m_inv_pip_pim",&m_inv_pip_pim);
  //background->Branch("miss_mass_kp",&miss_mass_kp);
