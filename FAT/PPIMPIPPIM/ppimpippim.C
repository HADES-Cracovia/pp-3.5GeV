...
void ppimpippim::Loop()
{
 ...
 TFile f("output_histograms.root","RECREATE");
  
  const int xi2n=40;
  const int xi2step=30;
  const int xi2min=200;
  ...
  for(int i=0;i<xi2n;i++)                                                                                                                                                                                   
	...
	char hname2[20]; 
        ...
        sprintf(htitle1,"p #pi^{-} for chi^{2} < %d; M_{#pi^{-} p}[MeV]", xi2min+i*xi2step);
        sprintf(hname1,"p_pim_chi2_%d",xi2min+i*xi2step);
        p_pim_mass[i]=new TH1F(hname1,htitle1,1000,1000,2000);

        sprintf(htitle2,"#pi^{+} #pi^{-} for chi^{2} < %d; M_{#pi^{-} #pi^{+}}[MeV]", xi2min+i*xi2step);
	...
	sig_bg_ppim[i]=new TF1(fname3,"gaus(0)+pol4(3)",1090,1500);
  ...
  for(int i=0;i<xi2n;i++)
    {
      ppimpippim::FitL1115(p_pim_mass[i],sig_ppim[i],bg_ppim[i],sig_bg_ppim[i]);
      ppimpippim::FitK0(pim_pip_mass[i],sig_pippim[i],bg_pippim[i],sig_bg_pippim[i]);
