...
bool HGraphCut::graphCut(HParticle *pPart)
{
   ...
   switch ( id )
   {
       ...
       case 3: return true; // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! always ok
	           ...
	           break;
       case 8: if (pip_cut) return pip_cut->IsInside( beta, mom );
	           break;
       case 9: if (pim_cut) return pim_cut->IsInside( beta, mom ); // same for pi-
       //case 9: if (pip_cut) return pip_cut->IsInside( beta, mom );
	           break;
       case 14: if (p_cut) return p_cut->IsInside( beta, mom );
	            break;
       case 45: return true;
