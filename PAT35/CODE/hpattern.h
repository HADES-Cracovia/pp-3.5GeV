...
using namespace CommonDefinitions;
   ...
   class HPattern 
   {
      public:
         ...
         HPattern(std::string name, EParticle p1, EParticle p2, EParticle p3, EParticle p4, EParticle p5, HOutputFile *ptr = 0) :
                           number(0), sId(name), prongNumber(5), ptrOut(0) {
#ifdef DEBUG
                               std::cout << "[DEBUG] HPattern 5: " << name << " particle: " << p1 << ", " << p2 << ", " << p3 << ", " << p4 << ", " << p5 << std::endl;
#endif
                               add(p1); add(p2); add(p3); add(p4); add(p5); out(ptr, &name);
                           }
         HPattern(std::string name, EParticle p1, EParticle p2, EParticle p3, EParticle p4, EParticle p5, EParticle p6, HOutputFile *ptr = 0) :
                           number(0), sId(name), prongNumber(6), ptrOut(0) {
#ifdef DEBUG
                               std::cout << "[DEBUG] HPattern 6: " << name << " particle: " << p1 << ", " << p2 << ", " << p3 << ", " << p4 << ", " << p5 << ", " << p6 << std::endl;
#endif
                               add(p1); add(p2); add(p3); add(p4); add(p5); add(p6); out(ptr, &name);
                           }
