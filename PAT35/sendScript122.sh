...
nFilesPerJob=2                                 # number of files to be analyzed by 1 job (default==1)
jobscript=${submissiondir}/jobScript.sh      # exec script (full path, call without dot, set it executable!)
outputdir=/hera/hades/user/${user}/apr35/PAT/122pions # outputdir for files AND logFiles
...
par2=${submissiondir}/anapp122pions             # optional par2 : executable
...
filelist=${currentDir}/pp35_122.list  # file list in local dir! not in submissiondir!!!
######################################################################
